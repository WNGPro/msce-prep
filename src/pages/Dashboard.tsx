              ...
              return (
                <div key={sv} className="card-elevated rounded-2xl p-4" onClick={() => navigate(`/tests?subject=${sv}`)} style={{ cursor: 'pointer' }}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {(stat?.testCount ?? 0) > 0 && (
                      <div className={`ml-auto font-bold font-display text-2xl ${gradeToColor(stat?.grade ?? 9)}`}>
                        {stat?.grade}
                      </div>
                    )}
                  </div>
                  {(stat?.testCount ?? 0) > 0 ? (
                    <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--border)' }}>
                      <div className="h-full rounded-full transition-all duration-700" style={{ width: `${stat?.avgScore ?? 0}%`, background: subj.color }} />
                    </div>
                  ) : (
